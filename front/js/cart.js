...
function panierVideCommande() { // si le localStorage est vide : affiche un panier vide
...
function panierVide() {
    if (objetPanier == 0) { // si le localStorage existe mais qu'il est vide, affiche un panier vide (en cas de suppression de produit manuellement ou via le LS)
        document.getElementsByTagName("h1")[0].innerHTML = "Votre panier est vide."
        document.getElementsByClassName("cart__price")[0].innerHTML = ""
        document.getElementsByClassName("cart__order")[0].innerHTML = ""
    }
...
// sélectionne la partie formulaire de la page panier
...
const prenomValide = function (prenomInput) {   // regex prénom (ne peut pas contenir d'espace, tirets autorisés)
    ...
    let prenom = rgxPrenom.test(prenomInput.value);
    if (prenom) {
...
const nomValide = function (nomInput) { // regex nom de famille (ne peut pas contenir d'espace, tirets autorisés)
    ...
    let nom = rgxNom.test(nomInput.value);
    if (nom) {
...
const adresseValide = function (adresseInput) { // regex adresse
    ...
    let adresse = rgxAdresse.test(adresseInput.value);
    if (adresse) {
...
const villeValide = function (villeInput) { // regex ville
    ...
    let ville = rgxVille.test(villeInput.value);
    if (ville) {
...
const emailValide = function (emailInput) { // regex email (ne peut pas contenir un seul caractère après le @)
    let rgxEmail = new RegExp("^[a-zA-Z0-9.-_]+[@]{1}[a-zA-Z0-9.-_]+[.]{1}[a-z]{2,10}$", "g");
    let email = rgxEmail.test(emailInput.value);
    if (email) {
    ...
    submit.addEventListener("click", (event) => {
        ...
        if (
            ...
            fetch("http://localhost:3000/api/products/order", post) // on fetch l'endroit ou l'on va envoyer les données, celles ci seront envoyés dans un array vide situé dans l'api
                ...
                .then(data => {
                    localStorage.clear(); // les données du local storage sont supprimées afin de faciliter une nouvelle commande et ne pas stocker les données
            ...
            alert("Le formulaire est incomplet ou comporte des erreurs"); // else si le formulaire ne correspond pas aux regex ou incomplet, n'envoie pas la commande
        }
    });
}
